...
# ArcFace reference landmark positions for 112×112 alignment
...
def detect_faces(img_rgb: np.ndarray,
    ...
    Returns list of dicts:
        {
          'bbox':      [x1, y1, x2, y2],
          'score':     float,
          'aligned':   np.ndarray (112x112 RGB) or None,
        }
    """
    ...
    for idx in keep:
        ...
        # map from padded-256 space → original image space
        box_orig = np.array([
            (b[0] - pl) / scale,
            (b[1] - pt) / scale,
            (b[2] - pl) / scale,
...
# =============================================================================
# Face Database (numpy .npz — เข้ากันได้กับ build_database.py)
# =============================================================================

class FaceDB:
    """
    Lightweight face database backed by a .npz file.
    Compatible with build_database.py from cavaface-onnx-float.
    """
    ...
    def search(self, query: np.ndarray, threshold: float = 0.45) -> tuple:
        ...
        best_name is None if no match above threshold.
        """
...
def build_database_from_folder(datasets_dir: str,
                                ...
                                db: FaceDB) -> None:
    """
    อ่านรูปจาก datasets/ แล้ว enroll ทุกคนก่อนกล้องเริ่มทำงาน
...
# =============================================================================
# HTML Template (same UI as web.py — no enroll modal)
# =============================================================================
...
#videoStream { width: 100%; background: #000; }
.faces-panel {
    background: var(--glass);
    padding: 20px;
    border: 1px solid var(--border);
...
def detection_thread(camera_id: int, threshold: float, skip_frames: int = 1) -> None:
    ...
    while True:
        ...
        if frame_count % (skip_frames + 1) == 1:
            ...
            # Deduplicate: one identity → one best face
            if len(faces) > 1:
                seen: dict[str, int] = {}
...
def main():
    ...
    # โมเดลทั้งสองใช้ external data file (.data) ที่ต้องอยู่ใน directory เดียวกัน
    # ต้อง chdir ไปที่ directory ของโมเดลก่อน แล้วค่อยโหลด
    orig_dir = os.getcwd()
